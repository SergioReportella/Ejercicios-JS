...
for (const user of users) {
    ...
    for (const sound in favoritesSounds) {
        ...
        if (contador[soundName]) {
            contador[soundName]++;
        } else {
            contador[soundName] = 1;
        }
    }

}

for (const soundName in contador) {
    console.log(`El sonido ${soundName} ha sido agregado a favoritos ${contador[soundName]} veces`);
};
